    ...
    const handleVerify = async () => {
        ...
        try {                // Check if admin token
            if (token === adminToken) {
                ...
                setSuccess(true);
                setMessage('Admin verified! Redirecting to /admin...');
                
                const redirectPath = sessionStorage.getItem('redirect_after_passkey') || '/admin';
                sessionStorage.removeItem('redirect_after_passkey'); // Clean up
            ...
            console.log('📥 Server passkey verification response:', data);            if (data.success) {
                loginSmtp(token, false); // Corrected: isAdmin should be false for a passkey
                ...
                // If a specific redirect was requested AND it's NOT an admin path, use it.
                // Otherwise, default to /smtps for passkey users.
                if (intendedRedirect && intendedRedirect !== '/admin') {
                    finalRedirectPath = intendedRedirect;
                } else {
                    finalRedirectPath = '/smtps'; 
                }
            ...
            onClose && onClose(); // Close popup on failure
        } finally {
